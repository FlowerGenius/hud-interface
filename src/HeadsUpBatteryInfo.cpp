...
void getBatteryInformation() {
	...
	while (battery_state.substr(battery_state.size() - 1) == " "
			or battery_state.substr(battery_state.size() - 1) == "\n")
...
void HeadsUpBatteryInfo::draw(int ax, int ay) {
	...
	glViewport(width - ax, height - TOP_MARGIN - BAT_HEIGHT, BAT_WIDTH,
	BAT_HEIGHT);
	...
	comp_text.ldraw(width - ax + (BAT_WIDTH / 6),
			height - TOP_MARGIN - BAT_HEIGHT * 0.8, 0, BAT_HEIGHT / 2);

	// Device Battery Monitor
	glViewport(width - ax, height - TOP_MARGIN - BAT_HEIGHT * 2 - height / 70,
	...
	comp_text.setText(std::to_string(dev_battery_life).erase(5));
	comp_text.setColour(167, 167, 255, 200);
	comp_text.ldraw(width - ax + (BAT_WIDTH / 6),
	...
	// Computer icon
	glViewport(width - ax - RIGHT_MARGIN - 10, height - TOP_MARGIN - BAT_HEIGHT,
			30, BAT_HEIGHT);
	{
		glBegin(GL_POLYGON); //We want to draw a map, i.e. shape with four bevel sides
		glColor4f(0.0, 0.6, 0.6, 0.9);
	...
	// Device Icon
	glViewport(width - ax - RIGHT_MARGIN - 10,
